...
## FT stuffs
from freqtrade.strategy import IStrategy , merge_informative_pair , stoploss_from_open , IntParameter , \
...
class CryptoFrog(IStrategy):
    ...
    sell_triggers = CategoricalParameter(['superabs' , 'stricter' , 'strict' , 'loose'] , space='sell' ,
                                         default='superabs' , optimize=False)
    ...
    # Dynamic ROI
    droi_trend_type = CategoricalParameter(['rmi' , 'ssl' , 'candle' , 'any'] , default='any' , space='sell' ,
                                           optimize=True)
    ...
    def populate_indicators(self , dataframe: DataFrame , metadata: dict) -> DataFrame:
        ...
        if self.timeframe == self.informative_timeframe:
            ...
            informatives_list = ['date' , 'open' , 'high' , 'low' , 'close' , 'volume' , 'emac' , 'emao' , 'srsi_d' ,
                                 'srsi_k' , 'msq_downtrend' , 'msq_uptrend' , 'msq_posidiv' , 'msq_negadiv' ,
                                 'ttmsqueeze' , 'atr' , 'roc' , 'rmi' , 'sroc' , 'ssl-dir' , 'rmi-up-trend' ,
                                 'candle-up-trend']
        ...
        if self.dp.runmode.value in ('backtest' , 'hyperopt'):
            ...
            self.custom_trade_info[metadata['pair']]['sroc_1h'] = dataframe[['date' , 'sroc_1h']].copy().set_index(
                'date')
            self.custom_trade_info[metadata['pair']]['ssl-dir_1h'] = dataframe[
                ['date' , 'ssl-dir_1h']].copy().set_index('date')
    ...
    def custom_stoploss(self , pair: str , trade: 'Trade' , current_time: datetime ,
        ...
        if (self.custom_info['stoploss_method'] == "simpletime") | (self.custom_info['stoploss_method'] == "any"):
            return self.custom_stoploss_simpletime(pair , trade , current_time , current_rate , current_profit , **kwargs)
        if (self.custom_info['stoploss_method'] == "riskreward") | (self.custom_info['stoploss_method'] == "any"):
            return self.custom_stoploss_riskreward(pair , trade , current_time , current_rate , current_profit , **kwargs)
        if (self.custom_info['stoploss_method'] == "roctime") | (self.custom_info['stoploss_method'] == "any"):
            return self.custom_stoploss_roctime(pair , trade , current_time , current_rate , current_profit , **kwargs)
        else:
    ...
    def min_roi_reached_dynamic(self , trade: Trade , current_profit: float , current_time: datetime ,
    ...
    def MadSqueeze(self , dataframe , period=34 , ref=13 , sqzlen=5):
        ...
        return df['msq_closema'] , df['msq_refma'] , df['msq_sqzma'] , df['msq_abs'] , df['msq_normabs'] , df[
            'msq_rollstd'] , df['msq_rollvar'] , df['msq_uptrend'] , df['msq_downtrend'] , df['msq_posidiv'] , df[
                   'msq_negadiv'] , df['msq_uptrend_buy']
